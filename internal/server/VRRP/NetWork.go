...
func NewIPIPv6AddrAnnouncer(nif *net.Interface) *IPv6AddrAnnouncer {
	var con, ip, errOfMakeNDPCon = ndp.Listen(nif, ndp.LinkLocal) //Dial(nif, ndp.LinkLocal)
	if errOfMakeNDPCon != nil {
		log.Fatalf("NewIPv6AddrAnnouncer: %v", errOfMakeNDPCon)
	}
	log.Infof("NDP client initialized, working on %v, source IP %v", nif.Name, ip)
...
func (nd *IPv6AddrAnnouncer) AnnounceAll(vr *VirtualRouter) error {
	for key := range vr.protectedIPaddrs {
		ip, _ := netip.AddrFromSlice(net.IP(key[:]))
		var multicastgroup, errOfParseMulticastGroup = ndp.SolicitedNodeMulticast(ip)
		if errOfParseMulticastGroup != nil {
			log.Errorf("IPv6AddrAnnouncer.AnnounceAll: %v", errOfParseMulticastGroup)
			...
			if errOfWrite := nd.con.WriteTo(msg, nil, multicastgroup); errOfWrite != nil {
				log.Errorf("IPv6AddrAnnouncer.AnnounceAll: %v", errOfWrite)
				return errOfWrite
			} else {
				log.Infof("send unsolicited neighbor advertisement for %v", net.IP(key[:]))
			}
		}
...
func (ar *IPv4AddrAnnouncer) AnnounceAll(vr *VirtualRouter) error {
	...
	for k := range vr.protectedIPaddrs {
		var ip, ok = netip.AddrFromSlice(net.IP(k[:]))
		if !ok {
			return fmt.Errorf("bad IP addres: %v", net.IP(k[:]))
		}
		packet.SenderHardwareAddr = vr.netInterface.HardwareAddr
		packet.SenderIP = ip
		packet.TargetHardwareAddr = BaordcastHADDR
		packet.TargetIP = ip
		log.Debug("send gratuitous arp for %v", net.IP(k[:]))
...
func NewIPv4AddrAnnouncer(nif *net.Interface) *IPv4AddrAnnouncer {
	if aar, errofDialARP := arp.Dial(nif); errofDialARP != nil {
		...
		log.Debug("IPv4 addresses announcer created")
...
func ipConnection(local, remote net.IP) (*net.IPConn, error) {
	...
	log.Infof("IP virtual connection established %v ==> %v", local, remote)
	return conn, nil
}
...
func joinIPv6MulticastGroup(con *net.IPConn, local, remote net.IP) error {
	...
	log.Infof("Join IPv6 multicast group %v on %v", remote, IF.Name)
	return nil
}
