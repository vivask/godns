...
func NewVirtualRouter(VRID byte, nif string, Owner bool, IPvX byte) *VirtualRouter {
	if IPvX != IPv4 && IPvX != IPv6 {
		log.Fatalf("NewVirtualRouter: parameter IPvx must be IPv4 or IPv6")
	}
	...
	if errOfGetIF != nil {
		log.Fatalf("NewVirtualRouter: %v", errOfGetIF)
	}
	...
	if preferred, errOfGetPreferred := findIPbyInterface(NetworkInterface, IPvX); errOfGetPreferred != nil {
		log.Fatalf("NewVirtualRouter: %v", errOfGetPreferred)
	} else {
...
func (r *VirtualRouter) AddIPvXAddr(ip net.IP) {
	...
	if _, ok := r.protectedIPaddrs[key]; ok {
		log.Errorf("VirtualRouter.AddIPvXAddr: add redundant IP addr %v", ip)
	} else {
...
func (r *VirtualRouter) RemoveIPvXAddr(ip net.IP) {
	...
	if _, ok := r.protectedIPaddrs[key]; ok {
		...
		log.Infof("IP %v removed", ip)
	} else {
		log.Errorf("VirtualRouter.RemoveIPvXAddr: remove inexistent IP addr %v", ip)
	}
}
...
func (r *VirtualRouter) fetchVRRPPacket() {
	for {
		if packet, errofFetch := r.iplayerInterface.ReadMessage(); errofFetch != nil {
			log.Errorf("VirtualRouter.fetchVRRPPacket: %v", errofFetch)
		} else {
...
func (r *VirtualRouter) stopMasterDownTimer() {
	log.Debug("master down timer stopped")
	if !r.masterDownTimer.Stop() {
		select {
		case <-r.masterDownTimer.C:
		default:
		}
		log.Debug("master down timer expired before we stop it, drain the channel")
	}
}
...
func (r *VirtualRouter) Enroll(transition2 transition, handler func()) bool {
	if _, ok := r.transitionHandler[transition2]; ok {
		log.Debug("VirtualRouter.Enroll(): handler of transition [%s] overwrited", transition2)
		r.transitionHandler[transition2] = handler
		return true
	}
	log.Debugf("VirtualRouter.Enroll(): handler of transition [%s] enrolled", transition2)
	r.transitionHandler[transition2] = handler
	return false
}
...
func (r *VirtualRouter) transitionDoWork(t transition) {
	...
	work()
	log.Debugf("handler of transition [%s] called", t)
	return
}
...
func largerThan(ip1, ip2 net.IP) bool {
	if len(ip1) != len(ip2) {
		log.Fatalf("largerThan: two compared IP addresses must have the same length")
	}
	for index := range ip1 {
...
func (r *VirtualRouter) eventLoop() {
	for {
		...
		case BACKUP:
			...
			case packet := <-r.packetQueue:
				if packet.GetPriority() == 0 {
					log.Infof("received an advertisement with priority 0, transit into MASTER state", r.vrID)
...
func (r *VirtualRouter) eventSelector() {
	for {
		...
		case INIT:
			...
			case event := <-r.eventChannel:
				if event == START {
					log.Debugf("event %v received", event)
					if r.priority == 255 || r.owner {
						log.Info("enter owner mode")
						r.sendAdvertMessage()
						if errOfarp := r.ipAddrAnnouncer.AnnounceAll(r); errOfarp != nil {
						...
						log.Debug("VR is not the owner of protected IP addresses")
						r.setMasterAdvInterval(r.advertisementInterval)
						//set up master down timer
						r.makeMasterDownTimer()
						log.Debug("enter BACKUP state")
